...
export const NewConversationModal = ({
  ...
  const [form, setForm] = useState({ phone: "", body: "", templateName: "" });
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div>
            ...
            <input
              ...
              placeholder="923001234567" className="input text-sm" required
            />
            <p className="text-[10px] text-slate-400 mt-1">
              Include country code without + or spaces. e.g. 923001234567
            </p>
          </div>

          {/* Toggle between free-form and template */}
          <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-100 rounded-lg">
            <input
              type="checkbox" id="useTemplate"
              checked={useTemplate}
              onChange={(e) => setUseTemplate(e.target.checked)}
              className="rounded border-slate-300 cursor-pointer"
            />
            <label htmlFor="useTemplate" className="text-xs text-amber-700 cursor-pointer">
          ...
          {useTemplate ? (
            <div>
              ...
              <input
                name="templateName" value={form.templateName} onChange={handleChange}
                placeholder="e.g. hello_world" className="input text-sm" required={useTemplate}
              />
              <p className="text-[10px] text-slate-400 mt-1">
            ...
            <div>
              ...
              <textarea
                name="body" value={form.body} onChange={handleChange as any}
                placeholder="Type your message..." rows={3}
                className="input text-sm resize-none" required={!useTemplate}
              />
              <p className="text-[10px] text-slate-400 mt-1">
                Free-form only works if the contact messaged you in the last 24hrs
              </p>
            </div>
          )}
