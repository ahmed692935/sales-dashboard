...
const navItems: NavItem[] = [
  { label: "Team Inbox", path: "/inbox", icon: <Inbox size={14} /> },
  { label: "Campaigns", path: "/campaigns", icon: <Megaphone size={14} /> },
  { label: "Automations", path: "/automations", icon: <Zap size={14} /> },
  { label: "Flows", path: "/flows", icon: <GitBranch size={14} /> },
  { label: "Contacts", path: "/contacts", icon: <Users size={14} /> },
  {
        ...
        <nav className="hidden lg:flex flex-1 items-center justify-center gap-1">
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              className={({ isActive }) =>
                `flex items-center gap-1.5 px-3 py-2 rounded-md text-[13px] font-medium transition-colors whitespace-nowrap ${
                  isActive
